...
(() => {
  ...
  function wireCheckout(card, costsByQty, buyEndpoint, upgradeId, currencyLabel, onBuyResult) {
    ...
    buyBtn.addEventListener('click', async () => {
      ...
      const res = await api(buyEndpoint, { upgradeId, amount: amt });
      if (res.error) {
      ...
      onBuyResult(res.data, toast, currencyLabel);
      buyBtn.disabled = false;
    });
  }
  ...
  function handleUpgradeBuy(d, toast) {
    if (d.status === 'success') {
      ...
      toast.textContent = 'Bought ' + d.amount + ' level(s) for ' + d.costLabel + ' credits.';
      refreshState();
      ...
      toast.className = 'toast error';
      toast.textContent = 'Not enough credits. Need ' + d.costLabel + ', have ' + d.creditsLabel + '.';
    } else {
      toast.className = 'toast error';
  ...
  function handleAscensionBuy(d, toast) {
    if (d.status === 'success') {
      toast.className = 'toast ok';
      toast.textContent = 'Bought ' + d.amount + ' level(s) for ' + d.costLabel + ' heavenly nuggies.';
      refreshState();
      ...
      toast.className = 'toast error';
      toast.textContent = 'Not enough heavenly nuggies. Need ' + d.costLabel + ', have ' + d.heavenlyNuggiesLabel + '.';
    } else {
      toast.className = 'toast error';
      toast.textContent = 'Unable to buy: ' + d.status;
    }
        ...
        (maxed
          ...
          : ('<div class="checkout-row">' +
              '<span class="qty">' +
                ...
                '<button type="button" data-qstep="1">+</button>' +
              '</span>' +
              '<span class="total-line">Total: <span class="amt" data-total></span> credits</span>' +
  ...
  function renderAscension() {
        ...
        (row.unlocked
          ? ('<div class="checkout-row">' +
              '<span class="qty">' +
                ...
                '<button type="button" data-qstep="1">+</button>' +
              '</span>' +
              '<span class="total-line">Total: <span class="amt" data-total></span> heavenly nuggies</span>' +
    ...
    if (!ascState.canAscend) {
      ...
      ascendStatus.textContent = 'You need at least 500 dinonuggies to ascend (you have ' + ascState.dinonuggiesLabel + ').';
  ...
  function bindAscend() {
    ...
    btn.addEventListener('click', async () => {
      ...
      if (d.status === 'too_few') {
        ...
        out.textContent = 'You need at least 500 dinonuggies. You have ' + d.dinonuggiesLabel + '.';
      } else if (d.status === 'success') {
        out.className = 'toast ok';
        out.textContent = 'Ascended! Gained ' + d.gainedLabel + ' heavenly nuggies. Ascension level: ' + d.ascensionLevel + '. New max upgrade level: ' + d.newMaxLevel + '.';
        await refreshState();
      }
      btn.disabled = false;
    });
