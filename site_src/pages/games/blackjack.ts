...
(() => {
  ...
  async function deal() {
    ...
    const d = data.data;
    showTable(d.amount.toLocaleString());
    clearHands();
  ...
  function showResult(d) {
    ...
    let sub = '';
    if (d.result === 'win') {
      sub = 'You won ' + (d.winnings != null ? Number(d.winnings).toLocaleString(undefined, { maximumFractionDigits: 2 }) : '') + ' mystic credits';
      if (d.streak) sub += ' • streak ' + d.streak;
    } else if (d.result === 'loss') {
      sub = 'You lost ' + (d.amount != null ? Number(d.amount).toLocaleString() : '') + ' mystic credits';
