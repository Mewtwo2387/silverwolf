...
const leaderboardsExtras = (nonce: string) =>
  raw(`
<script nonce="${nonce}">
(() => {
...
export function LeaderboardsPage(opts: {
  ...
  const { selected, result, error, nonce, lv999, user } = opts;

  const options = BOARD_OPTIONS.map(
    (o) =>
      html`<option value="${o.value}" ${o.value === selected ? 'selected' : ''}>
  ...
  const tableSection = (() => {
    ...
    if (!result)
      return html`<p class="text-fog-300 text-center font-mono my-8">&gt; SYSTEM STATUS: Awaiting query request...</p>`;
    if (result.rows.length === 0)
      return html`<p class="text-fog-300 text-center font-mono my-8">&gt; NO RECORDED DATA FOR: ${result.title}</p>`;
    return html`
      <h2 class="text-center font-mono text-lg tracking-wider text-accent mt-8 mb-4 uppercase">
                    ...
                    <div class="flex items-center gap-3">
                      ...
                      row.avatarURL
                        ? html`<img
                            ...
                            loading="lazy"
                            decoding="async"
                            class="w-8 h-8 rounded-full border border-ink-500 object-cover"
                          />`
                        : html`<div
                            class="w-8 h-8 rounded-full bg-ink-600 border border-ink-500 flex items-center justify-center font-mono text-xs text-fog-400"
                          >
                            ?
                          </div>`
                    }
                      <span class="font-mono font-medium text-fog-100 hover:text-accent transition-colors"
                        >${row.username}</span
                      >
                    </div>
                  </td>
                  <td class="py-3 px-4 font-mono text-right text-accent-light font-semibold">
                    ${row.valueTitle ? html`<span title="${row.valueTitle}">${row.valueLabel}</span>` : row.valueLabel}
                  </td>
                </tr>
              `,
            )}
  ...
  const body = html`
    ...
    <div class="max-w-[400px] mx-auto my-6">
      <form
        method="get"
        action="/leaderboards"
        class="text-center p-5 bg-ink-800/40 border border-ink-600 rounded-xl backdrop-blur-md relative overflow-hidden"
      >
        <div class="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-accent to-accent-pale"></div>
        <label for="board" class="block font-mono text-xs tracking-widest text-accent-light uppercase mb-3"
          >&gt; SYS.BOARD.QUERY</label
        >
    ...
    <div class="flex justify-center">
      <div class="min-w-[min(560px,100%)] overflow-x-auto">${tableSection}</div>
    </div>
