...
export function Navbar(active: NavActive | undefined, lv999?: boolean, user?: NavUser | null) {
  const base =
    'nav-link text-[0.95rem] px-[0.1rem] py-1 border-b-2 border-transparent transition-colors no-underline font-mono';
  ...
  const dockLink = (href: string, label: string, key: keyof typeof ICONS) => {
    ...
    return html`<a
      ...
      >${ICONS[key]}<span class="label">${label}</span></a
    >`;
  };
  ...
  return html`
    <nav id="site-nav" class="nav-surface font-mono">
      ...
      <div
        class="nav-window-header flex items-center justify-between px-[1.5rem] py-2 border-b border-[rgba(34,211,255,0.12)] select-none"
      >
      ...
      <div class="nav-window-content flex items-center justify-between py-[0.9rem] px-[clamp(1rem,4vw,3rem)]">
        ...
        <div class="nav-links relative font-mono" id="nav-links">
          ${link(aboutHref, 'About', 'about')} ${link('/leaderboards', 'Leaderboards', 'leaderboards')}
          ${link('/birthdays', 'Birthdays', 'birthdays')} ${link('/games', 'Games', 'games')}
          <span
            class="nav-underline nav-underline-grad absolute left-0 h-[2px] w-0 rounded-sm opacity-0 pointer-events-none"
            style="bottom:-2px;"
            aria-hidden="true"
          ></span>
        </div>
            ...
            ? html` <div id="nav-auth-desktop">
                <div class="nav-auth font-mono">
                  ...
                  <form action="/auth/logout" method="POST" style="display:inline;margin:0;">
                    ...
                    <button
                      type="submit"
                      class="nav-auth-link font-mono"
                      style="background:none;cursor:pointer;font-family:inherit;"
                    >
                      [logout]
                    </button>
                  </form>
            ...
            : html` <div id="nav-auth-desktop">
                <div class="nav-auth font-mono">
                  <a
                    ...
                    class="nav-login-link font-mono"
    ...
    <div id="nav-mobile" role="navigation" aria-label="Mobile navigation" style="display:none">
      ...
      ${dockLink(aboutHref, 'About', 'about')} ${dockLink('/leaderboards', 'Leaderboard', 'leaderboards')}
      ${dockLink('/birthdays', 'Birthdays', 'birthdays')} ${dockLink('/games', 'Games', 'games')}
    </div>
  `;
}
