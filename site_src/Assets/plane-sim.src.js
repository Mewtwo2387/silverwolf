...
import { buildClouds } from './plane-sim-clouds.js';
import { buildAirfield } from './plane-sim-airfield.js';
import { scatterVegetation, buildCoast } from './plane-sim-scatter.js';
...
`);
  ...
  buildCoast({ CFG, landGroup, addCyl, addBox });
  ...
  function applyWeather(name) {
    ...
    if (n !== 'storm') {
      ...
      cloudFlash = 0;
      if (stormDeckMat) stormDeckMat.emissive.setRGB(0, 0, 0);
  ...
  function stepWeather(dt) {
    ...
    if (cloudFlash > 0.004) {
      ...
      if (stormDeckMat) stormDeckMat.emissive.copy(FLASH_COL).multiplyScalar(cloudFlash * 0.9);
  ...
  function buildPlayer() {
    ...
    if (plane) {
      scene.remove(plane);
      // Free the outgoing airframe's GPU buffers. buildAircraft() makes fresh
      // geometry + materials per call, so without this an in-menu plane switch
      // leaks a whole aircraft's worth of them. Textures are shared via the
      // models cache and are deliberately NOT disposed here.
      plane.traverse((o) => {
        if (!o.isMesh) return;
        o.geometry?.dispose?.();
        const m = o.material;
