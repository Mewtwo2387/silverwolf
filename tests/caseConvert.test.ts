import { camelToSnake, snakeToCamel, snakeToCamelJSON, camelToSnakeJSON } from '../utils/caseConvert';
...
describe('snakeToCamelJSON', () => {
  test('changes JSON with snake_case keys to camelCase keys', () => {
    ...
    expect(snakeToCamelJSON({ snake_case_case: 'snek', snake_case: 'snake' })).toEqual({
      snakeCaseCase: 'snek',
      snakeCase: 'snake',
    });
  });
...
describe('camelToSnakeJSON', () => {
  test('changes JSON with camelCase keys to snake_case keys', () => {
    ...
    expect(camelToSnakeJSON({ camelCaseCase: 'caml', camelCase: 'cam' })).toEqual({
      camel_case_case: 'caml',
      camel_case: 'cam',
    });
  });
});
