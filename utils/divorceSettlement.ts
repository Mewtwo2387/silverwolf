...
interface DivorceSettlementResult {
  fixedFees: FeeEntry[];
  dynamicFees: FeeEntry[];
  initiator: UserShare;
  target: UserShare;
}
...
async function divorceSettlement(
  ...
  const initiatorDinonuggies: number = await client.db.user.getUserAttr(initiatorId, 'dinonuggies');
  const initiatorCredits: number = await client.db.user.getUserAttr(initiatorId, 'credits');
  const targetDinonuggies: number = await client.db.user.getUserAttr(targetId, 'dinonuggies');
  const targetCredits: number = await client.db.user.getUserAttr(targetId, 'credits');
...
export default divorceSettlement;
