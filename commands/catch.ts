  ...
  async run(interaction: any): Promise<void> {
    ...
    if (!currentPokemon) {
      await interaction.editReply({
        embeds: [new EmbedBuilder().setTitle("There's no pokemon to catch").setColor('#FF0000')],
      });
      return;
    }
    if (currentPokemon.toLowerCase() === pokemonName.toLowerCase()) {
      ...
      await interaction.editReply({
        embeds: [new EmbedBuilder().setTitle("There's no pokemon with that name to catch").setColor('#FF0000')],
      });
    }
  }
}
