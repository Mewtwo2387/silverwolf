...
class Snipe extends Command {
  ...
  async handleEditedSnipe(interaction: any, count: number): Promise<void> {
    const { editedMessages } = this.client;
    let total = 0;

    editedMessages.some((msg: any) => {
  ...
  async handleDeletedSnipe(interaction: any, count: number): Promise<void> {
    const { deletedMessages } = this.client;
    let total = 0;

    deletedMessages.some((msgData: any) => {
