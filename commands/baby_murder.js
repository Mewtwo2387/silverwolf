...
class BabyMurder extends Command {
  ...
  async run(interaction) {
    ...
    const lastMurder = await this.client.db.getUserAttr(interaction.user.id, 'lastMurder');
    const now = new Date();
    ...
    if (Math.random() < 0.5) {
      ...
      await this.client.db.setUserAttr(interaction.user.id, 'lastMurder', Date.now());
      await this.client.db.addUserAttr(interaction.user.id, 'murderSuccess', 1);
      await interaction.editReply({
...
<@${baby.motherId}> <@${baby.fatherId}> look at this murderer`,
      ...
      await this.client.db.setUserAttr(interaction.user.id, 'lastMurder', Date.now());
      await this.client.db.addUserAttr(interaction.user.id, 'murderFail', 1);
      await interaction.editReply({
