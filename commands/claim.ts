  ...
  async run(interaction: any): Promise<void> {
    try {
        ...
        await interaction.editReply({
          embeds: [
            new Discord.EmbedBuilder()
              ...
              .setDescription(
                `You now have ${format(result.previousDinonuggies + result.amount)} dinonuggies. You broke your streak of ${result.previousStreak} days.`,
              )
              .setColor('#83F28F')
        ...
        await interaction.editReply({
          embeds: [
            new Discord.EmbedBuilder()
              ...
              .setTitle(result.title)
              .setDescription(
                `You now have ${format(result.previousDinonuggies + result.amount)} dinonuggies. You are on a streak of ${result.previousStreak + 1} days.`,
              )
              .setImage(result.imageUrl)
              .setFooter({
                text: `dinonuggie | ${result.footer}`,
                iconURL: 'https://drive.google.com/thumbnail?id=1oVDRweQoYLU6YfB01LWZpTFQiBS1fRRa',
              }),
          ],
        });
      ...
      if ((await this.client.db.globalConfig.getGlobalConfig('banned')) === 'removed') {
        const embed = new Discord.EmbedBuilder().setColor('Green').setTitle('Welcome back!')
