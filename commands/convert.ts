...
class Convert extends Command {
  ...
  async run(interaction: any): Promise<void> {
    ...
    function convertUnit(val: number, from: string, to: string, category: string): number | string {
    ...
    await interaction.editReply({ content: `${value} ${fromUnit} is ${(result as number).toFixed(2)} ${toUnit}` });
  }
}
