...
class SexStart extends Command {
  constructor(client: any) {
    super(
      client,
      'start',
      'Start a sex session with a user',
      [
        {
          name: 'user',
  ...
  async run(interaction: any): Promise<void> {
    ...
    if (targetId === userId) {
      await interaction.editReply({
        embeds: [
          new Discord.EmbedBuilder().setColor('#AA0000').setTitle('@everyone')
...
If it occurs, you will be warned, then additional occurrences will be dealt with at the discretion of modstaff. Thanks.`),
    ...
    if (targetUser.bot && !modAbooz) {
      await interaction.editReply({
        embeds: [new Discord.EmbedBuilder().setColor('#AA0000').setTitle('Clank Clank Clank')],
      });
      return;
    }
      ...
      await interaction.editReply({
        embeds: [
          new Discord.EmbedBuilder()
            ...
            .setDescription(
              `You are already fucking <@${this.client.sexSessions.find((session: any) => session.hasUser(userId)).otherUser(userId)}>!`,
            )
      ...
      await interaction.editReply({
        embeds: [
          new Discord.EmbedBuilder()
            ...
            .setDescription(
              `<@${targetId}> is already fucking <@${this.client.sexSessions.find((session: any) => session.hasUser(targetId)).otherUser(targetId)}>!`,
            )
    ...
    const row = new Discord.ActionRowBuilder().addComponents(
      new Discord.ButtonBuilder().setCustomId('acceptSex').setLabel('Consent').setStyle(Discord.ButtonStyle.Success),
      new Discord.ButtonBuilder().setCustomId('rejectSex').setLabel('Ew no').setStyle(Discord.ButtonStyle.Danger),
    );
    ...
    collector.on('collect', async (i: any) => {
      ...
      if (i.user.id !== targetId && !modAbooz) {
        const responses = ['You wanna... threesome?', '~~Gamebang~~ Gangbangs are not allowed here!'];
