...
class Profile extends Command {
  ...
  async run(interaction: any): Promise<void> {
    ...
    const filter = (i: any) => i.customId === 'categorySelect' && i.user.id === interaction.user.id;
    const collector = interaction.channel.createMessageComponentCollector({ filter, time: 60000 });

    collector.on('collect', async (i: any) => {
      await i.deferUpdate();

      let detailsEmbed: Discord.EmbedBuilder | undefined;
      ...
      switch (i.values[0]) {
        case 'currency':
          ...
          break;
        case 'levels':
          detailsEmbed = this.createLevelsEmbed(user, ascensionLevel, maxLevel, username, avatarURL);
          break;
        case 'claims':
  ...
  createCurrencyEmbed(credits: any, user: any, username: string, avatarURL: string) {
...
${getNuggieNuggieMultiplierInfo(user.nuggieNuggieMultiplierLevel, INFO_LEVEL.THIS_LEVEL)}
  ...
  async createClaimsEmbed(
    user: any,
    nextClaim: number,
    nuggieStreakMultiplier: number,
    nuggieFlatMultiplier: number,
    nuggieCreditsMultiplier: number,
    log2Credits: number,
    username: string,
    avatarURL: string,
    pokemonCount: number,
    log2Nuggies: number,
    nuggiePokemonMultiplier: number,
    nuggieNuggieMultiplier: number,
  ) {
    return new Discord.EmbedBuilder()
      .setColor('#00AA00')
      .setTitle(`${username}'s Profile`)
  ...
  createGamblingEmbed(user: any, username: string, avatarURL: string) {
    return new Discord.EmbedBuilder()
      .setColor('#00AA00')
      .setTitle(`${username}'s Profile`)
      .setThumbnail(avatarURL)
...
**Max Streak:** ${user.rouletteMaxStreak}
  ...
  createOthersEmbed(user: any, username: string, avatarURL: string) {
    return new Discord.EmbedBuilder()
      .setColor('#00AA00')
      .setTitle(`${username}'s Profile`)
      .setThumbnail(avatarURL)
...
**Last Murder:** ${user.lastMurder ? new Date(user.lastMurder).toLocaleString() : 'Never'}
  ...
  createPokemonsEmbed(pokemonList: string, username: string, avatarURL: string) {
    return new Discord.EmbedBuilder()
      .setColor('#00AA00')
      .setTitle(`${username}'s Profile`)
      .setThumbnail(avatarURL)
