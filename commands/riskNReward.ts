...
class RiskNReward extends Command {
  ...
  async run(interaction: any): Promise<void> {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `You can win ${winAmount.toFixed(2)}% of your initial bet. Success is at ${failureChance}%.\n\nWould you like to continue?`,
      )
    ...
    const row = new ActionRowBuilder().addComponents(
      ...
      new ButtonBuilder().setCustomId('stepOut').setLabel('No balls').setStyle(ButtonStyle.Danger),
    );
    ...
    collector.on('collect', async (i: any) => {
      if (i.customId === 'continue') {
        ...
        if (rng < parseFloat(failureChance)) {
          ...
          await i.update({
            embeds: [
              embed.setDescription(`Aw, you lost! You lost ${lostAmount.toFixed(2)} credits.`).setColor('#FF0000'),
            ],
            components: [],
          });
          collector.stop();
          ...
          await i.update({
            embeds: [
              embed
                .setDescription(
                  `Congratulations! You won ${winnings.toFixed(2)} credits with a ${winAmount.toFixed(2)}% chance!`,
                )
                .setColor('#00FF00'),
            ],
            components: [],
        ...
        await i.update({
          embeds: [
            embed
              .setDescription(`You chose to step out. You lost ${entranceFee.toFixed(2)} credits as an entrance fee.`)
              .setColor('#0000FF'),
          ],
          components: [],
        });
    ...
    collector.on('end', async () => {
      if (!interaction.replied || !interaction.deferred) {
        ...
        await interaction.editReply({
          embeds: [new EmbedBuilder().setColor('#AA0000').setTitle(`You took too long and lost ${amount} credits!`)],
          components: [],
        });
      }
    });
