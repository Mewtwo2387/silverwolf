...
import divorceSettlement from '../utils/divorceSettlement';
...
class MarriageDivorce extends Command {
  ...
  async run(interaction: any): Promise<void> {
    ...
    const filter = (i: any) => i.customId === 'confirmDivorce' || i.customId === 'cancelDivorce';
    ...
    collector.on('collect', async (i: any) => {
      ...
      if (i.customId === 'confirmDivorce') {
        ...
        const fixedFeesBreakdown = settlement.fixedFees
          .map((fee: any) => `- **${fee.name}:** ${fee.dinonuggies} dinonuggies, ${fee.credits} credits`)
          .join('\n');
