  ...
  async run(interaction: any): Promise<void> {
    ...
    if (betType === 'number' && (Number.isNaN(betValue) || betValue < 0 || betValue > 36 || betValue === null)) {
      await interaction.editReply({
        embeds: [new Discord.EmbedBuilder().setColor('#AA0000').setTitle('Invalid number. Must be between 0 and 36')],
      });
      return;
    }
    ...
    await interaction.editReply({
      embeds: [
        new Discord.EmbedBuilder()
          ...
          .setTitle(
            `You bet ${format(amount)} mystic credits and ${result.isWin ? `won ${format(result.winnings)} mystic credits!` : 'lost!'}\n`,
          )
