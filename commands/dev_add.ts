...
class Add extends DevCommand {
  ...
  async run(interaction: any): Promise<void> {
    ...
    if (attr === 'dinonuggiesLastClaimed') {
      if (amountString === '-1d') {
        ...
        await interaction.editReply({
          embeds: [new Discord.EmbedBuilder().setColor('#00AA00').setTitle(`Set ${attr} to 1 day ago for ${user.tag}`)],
        });
        return;
      }
      if (amountString === '-2d') {
        ...
        await interaction.editReply({
          embeds: [
            new Discord.EmbedBuilder().setColor('#00AA00').setTitle(`Set ${attr} to 2 days ago for ${user.tag}`),
          ],
        });
        return;
      }
    ...
    await interaction.editReply({
      embeds: [
        new Discord.EmbedBuilder().setColor('#00AA00').setTitle(`Added ${format(amount)} ${attr} to ${user.tag}`),
      ],
    });
  }
}
