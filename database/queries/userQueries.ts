const userQueries = {
  ...
  // add user attribute
  ADD_USER_ATTR: (attr: string): string => `
    UPDATE User SET ${attr} = ${attr} + ? WHERE id = ?
  `,

  // set user attribute
  SET_USER_ATTR: (attr: string): string => `
  ...
  GET_EVERYONE_ATTR: (attr: string): string => `
    SELECT
      id, ${attr}
    FROM User
    WHERE ${attr} <> 0
  ...
  GET_EVERYONE_ATTR_LIMIT: (attr: string, limit: number, offset: number): string => `
    SELECT
      id, ${attr}
    FROM User
    WHERE ${attr} <> 0
  ...
  GET_EVERYONE_ATTR_COUNT: (attr: string): string => `
    SELECT COUNT(*) as count FROM User WHERE ${attr} <> 0
  `,

  GET_EVERYONE_RELATIVE_NET_WINNINGS: (type: string): string => `
    SELECT
      id,
  ...
  GET_EVERYONE_RELATIVE_NET_WINNINGS_LIMIT: (type: string, limit: number, offset: number): string => `
    SELECT
      id,
      (${type}_relative_won - ${type}_times_played) as relative_won
    FROM User
  ...
  GET_EVERYONE_RELATIVE_NET_WINNINGS_COUNT: (type: string): string => `
  ...
  GET_EVERYONE_RELATIVE_NET_WINNINGS_ALL_LIMIT: (limit: number, offset: number): string => `
    SELECT
      id,
      (slots_relative_won + blackjack_relative_won + roulette_relative_won - slots_times_played - blackjack_times_played - roulette_times_played) as relative_won
    FROM User
