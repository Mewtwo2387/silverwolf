...
class Database {
  ...
  async nameBaby(babyId, name) {
    const query = 'UPDATE Baby SET name = ? WHERE id = ?';
    const result = await this.executeQuery(query, [name, babyId]);
    const baby = await this.getBabyFromId(babyId);
  ...
  async getBabyFromId(babyId) {
    const query = 'SELECT * FROM Baby WHERE id = ?';
    const row = await this.executeSelectQuery(query, [babyId]);
    return row;
  }

  async getBabies(motherId, fatherId) {
    const query = 'SELECT * FROM Baby WHERE mother_id = ? AND father_id = ?';
    const rows1 = await this.executeSelectAllQuery(query, [motherId, fatherId]);
    const rows2 = await this.executeSelectAllQuery(query, [fatherId, motherId]);
    return [...rows1, ...rows2];
  }

  async getAllBabies() {
    const query = 'SELECT * FROM Baby';
    const rows = await this.executeSelectAllQuery(query);
    return rows;
  }
  ...
  async updateBabyStatus(babyId, status) {
    const query = 'UPDATE Baby SET status = ? WHERE id = ?';
  ...
  async updateBabyJob(babyId, job, pingerTarget = null, pingerChannel = null) {
    if (pingerTarget) {
      const query = 'UPDATE Baby SET job = ?, pinger_target = ?, pinger_channel = ? WHERE id = ?';
      ...
      const query = 'UPDATE Baby SET job = ? WHERE id = ?';
  ...
  async addBabyAttr(babyId, attr, value) {
    const query = `UPDATE Baby SET ${attr} = ${attr} + ? WHERE id = ?`;
  ...
  async getBabyAttr(babyId, attr) {
    const query = `SELECT ${attr} FROM Baby WHERE id = ?`;
    const row = await this.executeSelectQuery(query, [babyId]);
    return row[attr];
  }

  async setBabyAttr(babyId, attr, value) {
    const query = `UPDATE Baby SET ${attr} = ? WHERE id = ?`;
    await this.executeQuery(query, [value, babyId]);
    const baby = await this.getBabyFromId(babyId);
    log(`Updated baby ${babyId} ${attr} to ${value}. Mother: ${baby.mother_id}, Father: ${baby.father_id}, Status: ${baby.status}`);
  }
  ...
  async levelUpBaby(babyId) {
    const query = 'UPDATE Baby SET level = level + 1 WHERE id = ?';
  ...
  async updateBabyBirthday(babyId) {
    const query = 'UPDATE Baby SET born = CURRENT_TIMESTAMP WHERE id = ?';
    await this.executeQuery(query, [babyId]);
    const baby = await this.getBabyFromId(babyId);
